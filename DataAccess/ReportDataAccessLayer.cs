...
{
    public class ReportDataAccessLayer : MysqlDataAccessLayer
    {
        ...
        public ReportDataAccessLayer(IConfiguration configuration, IHttpContextAccessor httpContextAccessor) : base(configuration, httpContextAccessor)
        {
        ...
        public DataTable? CustomerSearchVMNList(string? Customer_id)
        {
            try
            {
                 //`web_get_vmn_from_reqid_dropdown`(in request_id varchar(20))
                using (MySqlCommand cmd = new MySqlCommand("web_get_vmn_from_reqid_dropdown"))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@request_id", MySqlDbType.VarChar, 20).Value = Customer_id;
        ...
        public DataTable? VMNReport(string? vmn, string? status, string? date,string? Customer_Id)
        {
            string? sts_out = ""; string? resp = "";
            try
            {
//                 `web_get_vmn_rpt_main`(in lcsms_id varchar(20), 
//in v_status varchar(20), in created_date varchar(20),
//out n_status varchar(20), out response varchar(20))
                using (MySqlCommand cmd = new MySqlCommand("web_get_vmn_rpt_main"))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    cmd.Parameters.Add("@customer_id", MySqlDbType.VarChar, 20).Value = Customer_Id;
        ...
        public SummaryResponseModal? SummaryReport(SummaryModal model)
        {
            SummaryResponseModal model1 = new SummaryResponseModal();
           
            try
            {
//                PROCEDURE `web_get_summary_rpt_main`(IN n_customer VARCHAR(200),
...
//OUT n_failure_count BIGINT)
                using (MySqlCommand cmd = new MySqlCommand("web_get_summary_rpt_main"))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@n_customer", MySqlDbType.VarChar, 200).Value = model.customer;
        ...
        public DataTable? DetailReport(string? customer, string? vmn, string? date)
        {
           
            try
            {
//                PROCEDURE `web_get_detail_rpt_main`(in n_customer varchar(200),
//in n_vmn varchar(200), in n_date varchar(200))
                using (MySqlCommand cmd = new MySqlCommand("web_get_detail_rpt_main"))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@n_customer", MySqlDbType.VarChar, 20).Value = customer;
                     cmd.Parameters.Add("@v_status", MySqlDbType.VarChar, 20).Value = vmn;
                    cmd.Parameters.Add("@created_date", MySqlDbType.VarChar, 20).Value = date;
        ...
        public DataSet? CustomerDropDown()
        {
            try
            {
                using (MySqlCommand cmd = new MySqlCommand("web_get_customer_dropdown_vmnlist"))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    using (MySqlConnection con = new MySqlConnection(connStr))
                    {
